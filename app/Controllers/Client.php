...
*/
        ...
        $identifiantsite = $modelIdent->retournerIdentifiant();
        $modelCli = new ModeleClient();
        $Utilisateur = $modelCli->retournerClientNumero($session->get('id'));
      ...
      */
        $pbx_repondre_a = php_url('traitementRetourPaybox.php'); // php_url ajouté dans helper 'assets_helper.php'
      ...
      */
      ...
      PayBox dispose de plusieurs serveurs, dans ce qui suit on cherche un serveur de prod. Opérationnel pour répondre à notre demande */
        ...
        $DonneesInjectees['client'] =  $modelCli->retournerClientNumero($session->get('id'));
        $modelCat = new ModeleCategorie();
        $DonneesInjectees['categories'] = $modelCat->retournerCategories();

        echo view('templates/header', $DonneesInjectees);
        echo view('Client/validationCommande');
        echo view('templates/footer');
    }

    function paiementRefuse() // simple redirection
    {
        echo '<script language=javascript>
      alert("Votre paiement a été refusé");
      </script> ';
        return redirect()->to('client/validationCommande');
    }

    function paiementAnnule() // simple redirection
    {
        return redirect()->to('client/validationCommande');
    }
   ...
   */
...

    {
        ...
        foreach (array_values(session('cart')) as $item) {
            ...
            $modelLig->ajouterLigne($donneesAInsererLi);
            $modelProd = new ModeleProduit();
            $infoproduit= $modelProd->retournerProduits($item['id']);
        ...
        $user = $modelCli->retournerClientNumero($session->get('id'));
   ...
   margin-bottom: 50px;
    ...
    public function historiqueCommandes()
    {
      $session = session();
        ...
        $DonneesInjectees['client'] = $modelCli->retournerClientNumero($session->get('id'));
        $modelComm = new ModeleCommande();
        $DonneesInjectees['commandes'] = $modelComm->retournerCommandesClient($session->get('id'));
        $modelCat = new ModeleCategorie();
        $DonneesInjectees['categories'] = $modelCat->retournerCategories();
        echo view('templates/header', $DonneesInjectees);
        echo view('Client/historiqueCommandes');
        echo view('templates/footer');
    }
    ...
    public function detailsCommande($nocommande = false)
    {
        ...
        $modelComm = new ModeleCommande();
        $DonneesInjectees['commande'] = $modelComm->retournerCommande($nocommande);
        $modelLig = new ModeleLigne();
        $DonneesInjectees['lignes'] = $modelLig->retournerLignes($nocommande);
        $modelCat = new ModeleCategorie();
        $DonneesInjectees['categories'] = $modelCat->retournerCategories();
        echo view('templates/header', $DonneesInjectees);
        echo view('Client/detailsCommande');
        echo view('templates/footer');
    }
    //fontion pour supprimer mon compte

      public function droitOublie($email)
      {
        $modelCat = new ModeleCategorie();
        $data_bis['categories'] = $modelCat->retournerCategories();
        $data['TitreDeLaPage'] = "Votre compte à bien été supprimeé";
        echo view('templates/header', $data_bis);
        $modelCli = new ModeleClient();
        $client= $modelCli->supprimerClient($email);
        log_message('error', $email);
        echo view('client/compteSupprimer', $data);
        echo view('templates/footer');
      }
}